...
object CommonEffects {
  ...
  fun <R> catchErrorSuspend(catcher: suspend CoroutineScope.(Throwable) -> R):
    ISagaEffectTransformer<R, R> = { SuspendCatchErrorEffect(it, catcher) }

  /** Create a [AsyncCatchErrorEffect] */
  @JvmStatic
  fun <R> catchErrorAsync(catcher: suspend CoroutineScope.(Throwable) -> Deferred<R>):
    ISagaEffectTransformer<R, R> = { AsyncCatchErrorEffect(it, catcher) }
  ...
  fun <P, R> mapSuspend(transformer: suspend CoroutineScope.(P) -> R):
    ISagaEffectTransformer<P, R> = { SuspendMapEffect(it, transformer) }

  /** Create a [AsyncMapEffect] */
  @JvmStatic
  fun <P, R> mapAsync(transformer: suspend CoroutineScope.(P) -> Deferred<R>):
    ISagaEffectTransformer<P, R> = { AsyncMapEffect(it, transformer) }

  /** Create a [PutEffect] */
  @JvmStatic
  fun <P> put(actionCreator: (P) -> IReduxAction):
    ISagaEffectTransformer<P, Any> = { PutEffect(it, actionCreator) }

  /** Create a [RetryEffect] */
  @JvmStatic
  fun <R> retry(times: Long): ISagaEffectTransformer<R, R> = { RetryEffect(it, times) }
  ...
  ): SagaEffect<R3> = ThenEffect(source1, source2, combiner)

  /** Create a [TimeoutEffect] */
  @JvmStatic
  fun <R> timeout(millis: Long): ISagaEffectTransformer<R, R> = { TimeoutEffect(it, millis) }
}
